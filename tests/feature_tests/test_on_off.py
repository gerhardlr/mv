...
def a_machine_that_is_on(proxy: Proxy):
    proxy.command_on()
...
def when_I_command_it_to_switch_off(proxy: Proxy):
    subscriber = Subscriber()
    proxy.subscribe(subscriber)
    proxy.command_off()
    return (proxy, subscriber)
...
def when_back_I_command_it_to_switch_off(proxy: Proxy):
    subscriber = Subscriber()
    proxy.subscribe(subscriber)
...
def then_the_server_should_be_off(command_results: tuple[Proxy, Subscriber]):
    proxy, subscriber = command_results
