...
def check_busy(method: Callable[P, T]) -> Callable[P, T]:
    def wrapper(*args: P.args, **kwds: P.kwargs) -> T:
        if state_machine_is_busy():
            raise StateMachineBusyError(f"{method.__name__} not allowed when state machine is busy")
...
class StateMachine:
    ...
    def assert_ready(self):
        if state_machine_is_busy():
            raise StateMachineBusyError(f"machine is busy and can not be commanded")
